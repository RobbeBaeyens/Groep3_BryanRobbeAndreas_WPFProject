    ...
    public partial class Event : BasisKlasse
    {
        public override string this[string columnName]
        {
            get
            {
                ...
                {
                    return "Selecteer een event type!";
                }*/
                ...
                if (columnName == "Startuur" && !Regex.IsMatch(Startuur, @"([01]?\d|2[0-3])u[0-5][0-9]?\d"))
                {
                    return "Vul een correct startuur in! HHuMM ==> vb. 12u30";
                }
                if (columnName == "Einduur" && !Regex.IsMatch(Startuur, @"([01]?\d|2[0-3])u[0-5][0-9]?\d"))
                {
